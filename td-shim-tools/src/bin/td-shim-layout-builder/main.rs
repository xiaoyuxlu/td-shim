...
use td_shim_tools::layout_builder::{
    self, parse_csv, parse_json, parse_layout_config, MemoryRegions,
};
...
struct Cli {
    ...
    config: String,
    /// Specify config file type, e.g. csv, json, layout-config
...
fn main() {
    ...
    let memory_regions = match cli.config_type {
        ConfigType::CSV => parse_csv::parse_memory(memory_regions, cli.config),
